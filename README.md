...
4. **delegated-execution** — タスクごとに新しい `codex exec` セッションを起動。タスクごとに2段階レビュー（仕様準拠 + コード品質）。最終的に `codex exec` でレンズベースの実装レビューを実施。
...
| `delegated-execution` | タスクごとの Codex 実行とレビューゲート |
